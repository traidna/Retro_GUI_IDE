...
char vrch=95;
  ...
  //cputcxy(x1,y1,159);
  //cputcxy(x2,y1,159); 
  //gotoxy(x1+1,y2); putchar(hch);  
  //gotoxy(x2-1,y2);  putchar(hch);  

  for (i=x1+1 ; i<x2; ++i) {
	   //gotoxy(i,y1);  putchar(hch);  
	   cputcxy(i,y1,159);
	   //gotoxy(i,y2); putchar(hch);
  } 
  mt_on();
  for (i=x1+1 ; i<x2; ++i) {
  ...
  for(i=y1+1; i<y2; ++i){
	  gotoxy(x1,i); putchar(vlch);  
	  gotoxy(x2,i); putchar(vrch);  
  }
  mt_off();
...
unsigned box(tgui_Window *tmp) { 
 ...
 cleanscrn(tmp);
  ...
  if (tmp->color < 16 ) {
	 ...
	 mt_drawbox(tmp->x1, tmp->y1, tmp->x2, tmp->y2); 
  }	  
    
  cputsxy(tmp->x1+2,tmp->y1,tmp->title);
...
unsigned mt;
	...
	if (mt==0 ) drawbox(x, y, x+obj->width, y + obj->height);
    else mt_drawbox(x, y, x+obj->width, y + obj->height);
	...
	if (mt==0) {
       if (startid != 0 ) cputcxy(x+obj->width-1, y, '+');	
		 ...
		 if (startid != 0 )  {
             //cputsxy(3,20,"show"); cgetc();
			 mt_on(); gotoxy(x+obj->width, y+1);
             putchar(75); mt_off();
	      }
	}
	    if (item!=NULL ) {
            ...
            if (mt==0) {			
	            if (item != obj->firstitem ) {
				    cputcxy(x+obj->width, y+obj->height-1, '+');
			    }
		  	    else {cputcxy(x+obj->width, y+obj->height-1, '-'); }
			}
			else { 
			 if (item != obj->firstitem ) {
				    //cgetc();
				 	gotoxy(x+obj->width, y+obj->height-1); 
					mt_on(); putchar(74); mt_off();
			    }
		  	    else {
					gotoxy(x+obj->width, y+obj->height-1); 
                    mt_on(); putchar(95); mt_off();
			    }
            }
	   ...
	   switch(key){
		   ...
		   case CH_CURS_DOWN:
  			...
  			if (y==maxy ) {
				   ...
				   showlistbox(obj, ox-1, oy-1, win->color, obj->topid);
			   ...
			   if(mt==0) {
			      if (y == maxy && li->next->li_id ==0){ 
					...
					if (y == maxy && hobj->topid ==0){
						gotoxy(obj->x+obj->width+2, obj->y+obj->height-1);
						mt_on(); putchar(95); mt_off();
					}
					else if (y==maxy && obj->topid !=0) {
						gotoxy(obj->x+obj->width+2, obj->y+obj->height-1);
						mt_on(); putchar(75); mt_off();
                      //  cputsxy(3,20,"plb1"); cgetc();
					}
				}	
		   ...
		   case CH_CURS_UP:
			   ...
			   revers(0);
			   if(mt==0) {
			       if (y==oy && li->li_id==0) cputcxy(obj->x+obj->width, oy-1, '-');
...
int x, y, i, h;
   ...
   if (obj->objtype==label) {
	 ...
	 if (win->color <16 )drawbox(x,y,x+obj->width, y+obj->height) ; 
	 else mt_drawbox(x,y,x+obj->width, y+obj->height) ; 
   }
	  ...
	  for(h=1; h <= obj->height ; ++h){
	   ...
	   showlistbox(obj, x, y, win->color, 0);
