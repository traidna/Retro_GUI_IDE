...
int button_focus_color=COLOR_YELLOW;


int p4color(int color ) {
...
void savescrn(tgui_Window *win) {

unsigned x,y,ctr;

long mstart;  // changed or plus 4

    mstart = PEEK(88) + 256*PEEK(89);
...
ctr=0;
	...
	for (x = win->x1; x <= win->x2; ++x){
		for(y = win->y1; y <= win->y2; ++y){
		   win->scrnchar[ctr] = PEEK(mstart+(y*40)+x);
		  // printf(" %d %d", ctr, sptr[ctr]); cgetc();
		  // win->scrncolr[ctr] = COLOR_RAM[y*40+x];  // & 15;
		   //win->scrncolr[ctr] = PEEK(cstart+(y*40)+x);  // & 15;
		   ++ctr;
		    //gotoxy(3,22);
			//cprintf("     \0"); gotoxy(3,22);
	       // cprintf("%d", PEEK(mstart+(y*40)+x));
	        //cgetc();
	    };
	};
	cursor(1);
...
void restorescrn(tgui_Window *win) {
unsigned ctr;
unsigned x, y;
long mstart;
  ...
  mstart = PEEK(88) + 256*PEEK(89);
	...
	for (x=win->x1; x <= win->x2; ++x){
		for(y=win->y1; y <= win->y2; ++y){
		   //COLOR_RAM[(y*40)+x]=win->scrncolr[ctr];
		   POKE(mstart+(y*40)+x, win->scrnchar[ctr]);
		   //POKE(3072+(y*40)+x, win->scrnchar[ctr]);
		   ++ctr;
	    }
	}
}  // end of restorescrn()
	   ...
	   switch(key){
		   ...
		   case CH_CURS_DOWN:
			   ...
			   if (y == maxy && li->next->li_id ==0){ 
			       cputcxy(obj->x+obj->width, obj->y+obj->height+2, CH_HLINE);
			   }
		   ...
		   case CH_CURS_UP:
			   ...
			   if (y==oy && li->li_id==0) cputcxy(obj->x+obj->width, oy-1, CH_HLINE);
...
int x, y, i, h;
   ...
   else if (obj->objtype==radiobutton) {
	    cputsxy(x,y,obj->caption);
	   if (obj->state==0) cputc(119); // 119 open 113 filled
	   else cputc(113);
   }  
...
void (*canfp) (void *);
	...
	newobject(0,1,0,1,COLOR_WHITE| 0X60,0,caption,"\0", label, mwin,NULL);
	newobject(0,2,size,1,COLOR_WHITE| 0X60,0,"\0","\0", simpletext, mwin,NULL);
    newobject(5,4,19,1,COLOR_GREEN,0,"ok\0", "\0",button, mwin, okfp);  //&foo
	newobject(15,4,31,1,COLOR_RED ,0,"quit\0", "\0",button, mwin, canfp);  //&foo
...
char key;
	...
	//textcolor(p4color(win->color));         // set menu text color to match window it is in
	revers(1);
	   ...
	   switch (key) {
			...
			case CH_ENTER:
				...
				if(menu->firstmi != NULL) {
					...
					if (retval>=0) return ((menu->menu_id)+1)*10 + retval+1;	
                    else {
						key=0;
					...
					menu->fp(win);
					if (win->quitflag==0) key=0;
			        else {
                      key = CH_ENTER;
					  return ((menu->menu_id)+1)*10; 
					}					  
				}
