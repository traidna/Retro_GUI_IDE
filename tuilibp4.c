...
int p4color(int color ) {
	...
	switch (color){
		...
		case 10:
		    color=COLOR_RED | 0X40;
			break;
		case 11:
			color = COLOR_WHITE | 0X10;
			 break;
		case 12:
			color = COLOR_WHITE | 0X30;
			 break;
        case 7:
             color=color|0x50;
			 break;
		case 13:
             color=COLOR_GREEN|0x50;
			 break;
		case 14:
             color=COLOR_BLUE |0x30;
			 break;
		case 15:
			color = COLOR_WHITE | 0X50;
			 break;			 
...
int button_focus_color=COLOR_YELLOW | 0X50;
...
int size = win->x2-win->x1+1;
	...
	for (y=win->y1 ; y<=win->y2; ++y){
		...
		memcpy(win->scrncolr+(c*size), cs + (y*40)+win->x1, size);
		++c;
	}
...
int size = win->x2-win->x1+1; 
   	...
   	for (y=win->y1 ; y<=win->y2; ++y){
		...
		memcpy(cs + (y*40)+win->x1, win->scrncolr+(c*size), size);
		++c;
	}
	...
	do { 
	   ...
	   switch (key) {
			case CH_ENTER:
					...
					//gotoxy(2,20); cprintf("reval = %d qflag=%d", retval, win->quitflag);
